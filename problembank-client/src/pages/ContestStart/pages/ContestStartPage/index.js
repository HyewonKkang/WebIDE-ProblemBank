...
function DetailProblem(props) {
    ...
    useEffect(() => {
        if(problemsAllData){
            ...
            const problems1 = data.filter(element => element.level === '하')
            const problems2 = data.filter(element => element.level === '중')
            const problems3 = data.filter(element => element.level === '상')
            ...
            for(let i=0; i<5; i++) {
                ...
                if (randomIndexArray.indexOf(randomNum) === -1) {
                    ...
                    problems.push(problems1[randomNum])
                } else {
                    i--;
                }
            }
            ...
            for(let i=0; i<4; i++) {
                ...
                if (randomIndexArray.indexOf(randomNum) === -1) {
                    randomIndexArray.push(randomNum);
                    problems.push(problems2[randomNum])
                } else {
                    i--;
                }
            }

            problems.push(problems3[0])
            id = randomIndexArray[0]
                ...
                <div className="problem__detail--content">
                    <div className="tab__header">
                        <ul className="tab__header--content">
                            <li>남은 시간&nbsp;&nbsp;&nbsp;<Timer></Timer></li>
                        </ul>
                    </div>
                    <div className="wrapper__content">
                        <h3>{problems.indexOf(problem) + 1}. {problem.name}</h3>
                        ...
                        {/* <div className="pre-next-problem">
                            {
                                problems.length !== 0 ?
                                    <>
                                    <button onClick={() => props.history.push(`/problem/view?id=${problems.indexOf(problems.indexOf(problem)-1).id}`)} disabled={problem.id === problems[0].id} >Prev</button>&nbsp;
                                        <span>{problems.indexOf(problem)+1}/{problems.length}</span>&nbsp;
                                    <button onClick={() => onSubmit && props.history.push(`/problem/view?id=${problems.indexOf(problems.indexOf(problem)+1).id}`)} disabled={problem.id === problems[problems.length-1].id}>Next</button>
                                    </>
                                : ""
